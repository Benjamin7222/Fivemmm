...
function LoadUser(source, identifier)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT permission_group, permission_level, accounts, job, job_grade, job2, job2_grade, inventory, loadout, position FROM users WHERE identifier = @identifier', {
...
AddEventHandler('esx:dropInventoryItem', function(type, itemName, itemCount)
	...
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			--local pickupLabel = ('%s~s~ [~g~%s~s~]'):format(weapon.label, weapon.ammo)
