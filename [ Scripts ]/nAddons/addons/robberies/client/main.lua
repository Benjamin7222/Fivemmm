    ...
    NewKaizo.newThread(function()
        while currentlyRobbing do
            ...
            if IsPedDeadOrDying(PlayerPedId(), 1) then
                ...
                KaizoClientUtils.toServer("robberiesDiedDuring")
                Wait(3500)
                TriggerEvent("esx_ambulancejob:revive")
                ESX.ShowNotification("~y~Vous avez échoué, vous ne gagnez rien !")
            end
            Wait(4500)
        end
    end)
    NewKaizo.newThread(function()
        while currentlyRobbing do
            if not displayingObjectInfos then
                ...
                AddTextEntry("TIMER", ("Cambriolage en cours:~n~~y~Objets~s~: ~y~%s ~s~restants ~n~~s~[~y~%s$~s~]"):format(a, bagValue))
        ...
        while currentlyRobbing do
            ...
            for id, objectInfos in pairs(objects) do
                ...
                if dist <= objectInfos.iter then
                    ...
                    AddTextEntry("HELP", ("Appuyez sur ~INPUT_CONTEXT~ pour ramasser cet objet~n~~y~Objet~s~: %s~n~~y~Quantité~s~: %s~n~~y~Valeur~s~: %s~y~$~n~~y~Ramassage~s~: %s~y~s"):format(objectInfos.name, objectInfos.count, objectInfos.price, (objectInfos.timeToTake / 1000)))
                    ...
                    if IsControlJustPressed(0, 51) then
                        if not busy then
                            ...
                            NewKaizo.newWaitingThread(objectInfos.timeToTake, function()
                                ...
                                KaizoClientUtils.toServer("robberiesAddItem", objectInfos)
...
NewKaizo.netRegisterAndHandle("robberiesEnter", function(infosTable)
    ...
    for _, v in pairs(infosTable.objects) do
        ...
        RequestModel(prop)
        print(("[NewKaizo] Requesting model %s..."):format(itemInfos.prop))
        while not HasModelLoaded(prop) do
            Wait(1)
        end
        print(("[NewKaizo] Loaded item %s !"):format(itemInfos.prop))
    ...
    for id, v in pairs(infosTable.possibleOponents) do
        ...
        if v[5] ~= nil then
            TriggerServerEvent("core:achatarme", GetHashKey(v[5]))
...
NewKaizo.netRegisterAndHandle("robberiesStart", function()
    print("[LSExperience] Started music")
    PlayUrl("robb", "https://www.youtube.com/watch?v=_5vGqtbfuDY", 0.22, false)
end)

NewKaizo.netRegisterAndHandle("robberiesExit", function(exit)
    currentlyRobbing = false
    for k, v in pairs(objects) do
    ...
    KaizoClientUtils.toServer("robberiesRemoveBlip")
