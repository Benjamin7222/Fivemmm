...
setmetatable(Zone, {
    __call = function(_, location, type, color, onInteract, helpText, drawDist, itrDist, restricted, baseAllowed)
        ...
        self.zoneID = (#NewlandSZonesManager.list + 1)
        ...
        NewlandSZonesManager.list[self.zoneID] = self
        return self
    end
})
...
function Zone:interact(source)
    ...
    NLServerUtils.trace(("%s a intéragit avec la zone n°%s"):format(GetPlayerName(source),self.zoneID), NewKaizoPrefixes.zones)
end
