...
NewKaizo.newWaitingThread(100, function()
    badgeFromValue = {
...
NewKaizo.netRegisterAndHandle("openCloackroom", function(job)
                ...
                RageUI.ButtonWithStyle("Tenue de travail", "Vous permets d'enfiler votre tenue de travail", { RightBadge = badgeFromValue[isWithWorkingClothes] }, true, function(_, _, s)
                    if s and not isWithWorkingClothes then
                        ...
                        TriggerEvent('skinchanger:getSkin', function(skin)
                            TriggerEvent('skinchanger:loadClothes', skin, NewKaizoSharedCustomJobs[job].clothes[Job.grade_name][byIndex[skin.sex == 0]])
                        end)
                    end
                end)
            end, function()
