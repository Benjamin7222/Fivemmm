...
NewKaizo.netHandle("esxloaded", function()
    KaizoClientUtils.toServer("requestPredefinedNpcs")
            ...
            for npcId, npc in pairs(npcs.list) do
                ...
                if not npcs.list[npcId].npcHandle then
                    if #(pos - npcPos) <= genDist then
                        ...
                        if npc.frozen then
                            NewKaizo.newWaitingThread(1500, function()
...
NewKaizo.netRegisterAndHandle("npcPlaySound", function(npcId, speech, param)
...
---@param npc Npc
NewKaizo.netRegisterAndHandle("newNpc", function(npc)
    npcs.list[npc.id] = npc
end)

NewKaizo.netRegisterAndHandle("delNpc", function(npcId)
...
NewKaizo.netRegisterAndHandle("cbNpcs", function(incomingNpcs)
