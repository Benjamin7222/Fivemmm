                ...
                RageUI.IsVisible(MenuPrincipal, function()
                    ...
                    for k, v in pairs(Config.Shop) do 
                        RageUI.Button(v.name, nil, {RightLabel = "~r~"..ESX.Math.GroupDigits(v.price).."$"}, true, {
                            onSelected = function()
                                TriggerServerEvent("kPeche:AchatsItems", v.item, v.name, v.price, true)
                            end,
                        })    
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if dist <= 10 then
            wait = 0
            ESX.ShowHelpNotification("E", "Pour ranger le bateau") 
            if IsControlJustPressed(1,51) then
                if IsPedSittingInVehicle(GetPlayerPed(-1), Boat_Vehicle) then
                    MenuRetourLocation()
                else 
