...
Selldrugs.Items = {
	{item = "weed_pooch", label = "Pochon de Weed", props = "prop_weed_block_01", anim = {"mp_common", "givetake2_a"}, minmoney = 25, maxmoney = 50},
	{item = "coke_pooch", label = "Pochon de Coke", props = "prop_coke_block_half_b", anim = {"mp_common", "givetake2_a"}, minmoney = 50, maxmoney = 75},
	{item = "meth_pooch", label = "Pochon de Meth", props = "p_meth_bag_01_s", anim = {"mp_common", "givetake2_a"}, minmoney = 75, maxmoney = 100},
}
...
AddEventHandler('cDrugs:ActivDrugsSell', function()
    ...
    if not found then return ESX.ShowNotification("Vous devez avoir de la drogue sur vous.") end
    if not Selldrugs.OnSellDrugs then 
        ESX.ShowNotification("Vous avez activé la vente de drogues.")
        Selldrugs.OnSellDrugs = true 
    elseif Selldrugs.OnSellDrugs then 
        ESX.ShowNotification("Vous avez désactivé la vente de drogues.")
        Selldrugs.OnSellDrugs = false 
    end
end)
        ...
        if Selldrugs.OnSellDrugs and not IsPedInAnyVehicle(pPed) then 
            ...
            repeat 
                ...
                if dist <= 5.0 and Selldrugs:CanSellToPed(outEntity) then 
                    ...
                    if dist <= 2.5 then 
                        ESX.ShowHelpNotification("E", "pour vendre votre drogue")
                        ...
                        if IsControlJustPressed(0, Selldrugs.Key) then 
                            ...
                            if not minmoney and not maxmoney then 
                                ...
                                if chance >= 0 and chance <= 28 then
                                    ESX.ShowNotification("La personne ne veut pas de votre marchandise.")
                                    TriggerServerEvent("cDrugs:policeNotify", "police", pPos)
                                else
                                    if minmoney and maxmoney then 
                                        TriggerServerEvent('cDrugs:SellDrugs', item, label, math.random(minmoney, maxmoney), 1)
                                    else
                                        ESX.ShowNotification("Vous n'avez plus de drogues.")
                                        Selldrugs.OnSellDrugs = false 
                                    end
                                end
...
RegisterNetEvent('cDrugs:policeNotify')
AddEventHandler('cDrugs:policeNotify', function(alert)
