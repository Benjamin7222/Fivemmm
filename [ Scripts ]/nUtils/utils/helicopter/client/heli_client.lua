...
local as332_hash = GetHashKey("as332")
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerInPolmav() or IsPlayerInAS332()then
			...
			AddTextEntry('HelpNotification', "~INPUT_CONTEXT~ Activer la caméra de l'hélicopter\n~INPUT_AIM~ Activer la vision thermique\n~INPUT_JUMP~ Lock la caméra sur une voiture\n\n~INPUT_PARACHUTE_SMOKE~ Descendre en rappel")
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~Vous ne pouvez pas descendre en rappel de cette place")
...
function IsPlayerInAS332()
	local lPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, as332_hash)
end
