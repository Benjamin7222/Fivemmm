...
local vehMenu = menu.CreateMenu('', 'Véhicule du Unicorn :', 10, 10, 'commonmenu', 'interaction_bgd')
...
function OpenVehMenuUni()
    if VehUni then
        ...
        VehUni = true
    menu.Visible(vehMenu, true)
        CreateThread(function()
            while VehUni do
                Wait(1)
                menu.IsVisible(vehMenu, function()
                    menu.Button(" Ranger le Véhicule", nil, {RightLabel = "→→"}, true, { 
                        onSelected = function()
                    ...
                    menu.List(' Sortir un Véhicule', {'~b~T20~s~', '~b~Bisson~s~', '~b~Limousine~s~'}, Index.List1, nil, {}, true, {
