...
local open = false 
local mainMenu8 = RageUI.CreateMenu('~y~taxi', '~y~Interaction')
local subMenu8 = RageUI.CreateSubMenu(mainMenu8, "~y~Annonces", "~y~Interaction")
	...
	if inMission then
		...
		Citizen.CreateThread(
			function()
				...
				PlaySoundFrontend(-1, "Menu_Accept", "Phone_SoundSet_Default", 500)
				TriggerEvent(
				...
				while dst > 60.0 and inMission do
					...
					ShowNotification("Allez sur la zone GPS", 500)
					Wait(500)
				end
				...
				while dst > 10.0 and inMission do
					pCoords = GetEntityCoords(pPed)
					dst = GetDistanceBetweenCoords(pCoords, pos, true)
					ShowNotification("Approchez vous du client", 500)
					Wait(500)
				end
				...
				while not IsPedInAnyVehicle(ped, false) and inMission do
					Wait(1)
					ShowNotification("Attendre le client", 500)
					Wait(500)
				end
				...
				local bonus = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), posToGo, true) / 10
				...
				while dst > 10.0 and inMission do
					...
					ShowNotification("Allez-la ou le client vous à demandé", 500)
					Wait(500)
				end

				while GetEntitySpeed(pPed) > 3 do
					Wait(500)
					ShowNotification("Stopper le véhicule", 500)
				end
