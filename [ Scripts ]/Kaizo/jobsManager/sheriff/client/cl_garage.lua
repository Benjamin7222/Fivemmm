     ...
     if open then 
         ...
         while open do 
            RageUI.IsVisible(mainMenu6,function() 
                ...
                RageUI.Button(v.buttoname, nil, {RightLabel = "→→"}, ESX.PlayerData.job.grade >= v.disc , {
                    onSelected = function()
                        if not ESX.Game.IsSpawnPointClear(vector3(v.spawnzone.x, v.spawnzone.y, v.spawnzone.z), 10.0) then
                            ESX.ShowNotification("~y~sheriff\n~y~Point de spawn bloquée")
                        else
  ...
  while true do 
      ...
      if ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff' then
          for k in pairs(cfg_sheriff.Position.GarageVehicule) do 
              ...
              if dist <= 25.0 then 
                  wait = 0
                  DrawMarker(cfg_sheriff.MarkerType, pos[k].x, pos[k].y, pos[k].z-0.90, 0.0, 0.0, 0.0, -90.0,0.0,0.0, cfg_sheriff.MarkerSizeLargeur, cfg_sheriff.MarkerSizeEpaisseur, cfg_sheriff.MarkerSizeHauteur, cfg_sheriff.MarkerColorR, cfg_sheriff.MarkerColorG, cfg_sheriff.MarkerColorB, cfg_sheriff.MarkerOpacite, cfg_sheriff.MarkerSaute, true, p19, cfg_sheriff.MarkerTourne)  
              end
...
local open = false 
local mainMenu6 = RageUI.CreateMenu('~y~Garage', '~y~Hélico')
mainMenu6.Display.Header = true 
mainMenu6.Closed = function()
  open = false
end
         ...
         while open do 
            RageUI.IsVisible(mainMenu6,function() 
              ...
              RageUI.Separator("↓ Véhicules ↓")
                ...
                RageUI.Button(v.buttonameheli, nil, {RightLabel = "→→"}, ESX.PlayerData.job.grade >= 13 , {
                    onSelected = function()
...
RMenu.Add('sheriff', 'vehicle_option', RageUI.CreateMenu("Option véhicule", "Undefined for using SetSubtitle"))
RMenu:Get('sheriff', 'vehicle_option'):SetSubtitle("~y~Option véhicule")
                        ...
                        for i = 1,9 do
                            if DoesExtraExist(pVeh, i) then
                                if IsVehicleExtraTurnedOn(pVeh, i) then
                                    RageUI.Button("~y~Désactiver~s~ l'extra "..i, nil, {}, true, {
                                        onSelected = function()
                                            SetVehicleExtra(pVeh, i, true)
                                        end,
                                    }) 
                                else
                                    RageUI.Button("~y~Activer~s~ l'extra "..i, nil, {}, true, {
                                        onSelected = function()
...
Keys.Register('F4', 'F4', 'Ouvrir le menu géstion extra véhicule', function()
    if ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff' then
