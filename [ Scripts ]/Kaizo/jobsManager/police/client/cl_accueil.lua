...
function OpenMenuAccueilPolice() 
    if open then 
        ...
        RageUI.Button("Demander un agent du LSPD", nil, {RightLabel = "→"}, not codesCooldown5 , {
            onSelected = function()
            codesCooldown5 = true 
        TriggerServerEvent('Appel:police')
        ESX.ShowNotification('Votre message a bien été envoyé aux policer.')
        Citizen.SetTimeout(5000, function() codesCooldown5 = false end)
       end 
    })

        RageUI.Button("Avoir un rendez-vous", nil, {RightLabel = "→"}, true , {
            onSelected = function()
            end
        }, subMenu1)     
        ...
        RageUI.IsVisible(subMenu1, function()    
            ...
            RageUI.Button("Valider la Demande", "Merci de respecter les ambulancier.", { Color = {BackgroundColor = { 76, 175, 80, 50}} }, true, {
                onSelected = function()
                    if (nomprenom == nil or nomprenom == '') then
                        ESX.ShowNotification('Vous n\'a pas rempli ton Nom/Prénom')
                    elseif (numero == nil or numero == '') then
                        ESX.ShowNotification('Vous n\'a pas rempli ton Numéro')
                    elseif (heurerdv == nil or heurerdv == '') then
                        ESX.ShowNotification('Vous n\'a pas rempli l\'heure de Rendez-vous')
                    elseif (rdvmotif == nil or rdvmotif == '' or rdvmotif == "Motif") then
                        ESX.ShowNotification('Vous n\'a pas rempli le motif du rendez-vous')
                else
                    RageUI.CloseAll()
...
local npc = {
	{hash="csb_trafficwarden", x = 440.48596191406, y = -986.85864257813, z = 30.724328994751, a = 84.93338012695312},
}
