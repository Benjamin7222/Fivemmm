...
admins = {
    'license:4eb476b42bed74ba3527b01906b105d2d53a56d4', -- Kaizo
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('vSync:notify', source, 'Time is now ~g~frozen~s~.')
            else
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                TriggerClientEvent('vSync:notify', source, 'Changements métérologiques dynamiques ~g~desactivé~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'Changements métérologiques dynamiques ~g~activé~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1Vous n\'etes pas autorisé a utiliser cette commande.')
        end
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if blackout then
                ...
                TriggerClientEvent('vSync:notify', source, 'Blackout is now ~g~disabled~s~.')
            end
            TriggerEvent('vSync:requestSync')
        end
    end
