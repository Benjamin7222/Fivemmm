...
local function pointfolow(LfEJbh_,JD)
	...
	TaskVehicleFollowWaypointRecording(PlayerPedId(),JD,prop4,262144,0,546,-1,ti,false,1.25)
...
local function commencercarwash()
	...
	Wait(1000)
	fincarwashprops()
	--ShowAboveRadarMessage("~g~CarWash\n~s~Vous avez lavé votre ~g~"..GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(pVeh))).."~s~.")
		...
		for _,v in pairs(poscar) do
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				...
				if Vdist2(GetEntityCoords(PlayerPedId(), false), v) < 7 then
					attente = 1
					DrawText3D(v.x, v.y, v.z+0.2, "Appuyez sur ~g~E ~w~pour ~g~laver votre véhicule~w~.", 9)
...
function openWarcash()
	...
	while menu do
		...
		RageUI.IsVisible(menu, function()
            RageUI.Button('Néttoyer son véhicule (150$)', nil, {}, true, {onSelected = function()
