...
AddEventHandler("Kayce:AddVehToClient", function(id, name, plate, type)
    ...
    if xPlayer.getGroup() == "_dev" then
        if xLeJ ~= nil then
            ...
            xLeJ.showNotification("~g~Félicitation, il semblerait que "..xPlayer.name.." vous est ajouté un "..typeByName[type].." du nom de : "..name)
            xPlayer.showNotification("~g~Vous avez envoyé a "..xLeJ.name..", une "..typeByName[type].." du nom de : "..name)
        ...
        if (xSelected) then
            if xPlayer.getGroup() == "_dev" then
                MySQL.Async.execute([[
                    ...
                    if (result == 0) then
                        xPlayer.showNotification("Une erreur est survenue il semblerait que l'utilisateur seléctionné n'est pas valide ou inexistante")
                    else
                        xPlayer.showNotification("Action réussi, tous les véhicule de "..xSelected.name.." ont été supprimé !")
            ...
            if xPlayer.getGroup() == "_dev" then
                    MySQL.Async.execute([[
                        ...
                        if (result == 0) then
                            ...
                            xPlayer.showNotification("Action réussi, tous le véhicule sous la plaque "..selectedPlate.." a été supprimé de l'utilisateur détenteur !")
