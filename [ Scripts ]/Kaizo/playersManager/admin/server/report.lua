...
AddEventHandler("adminmenu:takeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("esx:showNotification", source, "[Report] Ce report n'est plus en attente de prise en charge")
        return
    end
    reportsTable[reportId].takenBy = GetPlayerName(source)
    reportsTable[reportId].taken = true
    if players[reportId] ~= nil then
        TriggerClientEvent("esx:showNotification", reportId, "[Report] Votre report a été pris en charge.")
    end
    notifyActiveStaff("[Report] Le staff "..GetPlayerName(source).." a pris en charge le report n°"..reportsTable[reportId].uniqueId)
...
AddEventHandler("adminmenu:closeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("esx:showNotification", source, "[Report] Ce report n'est plus valide")
        return
    end
    if players[reportId] ~= nil then
        TriggerClientEvent("esx:showNotification", reportId, "[Report] Votre report a été cloturé. N'hésitez pas à nous recontacter en cas de besoin.")
    end
    notifyActiveStaff("[Report] Le staff "..GetPlayerName(source).." a cloturé le report n°"..reportsTable[reportId].uniqueId)
...
RegisterCommand("report", function(source, args)
    ...
    if reportsTable[source] ~= nil then
        TriggerClientEvent("esx:showNotification", source, "[Report] Vous avez déjà un report actif.")
        return
    end
    ...
    notifyActiveStaff("[Report] Un nouveau report a été reçu. ID Unique: " .. reportsCount)
    TriggerClientEvent("esx:showNotification", source, "[Report] Votre report a été envoyé ! Vous serez informé quand il sera pris en charge et / ou cloturé.")
    updateReportsForStaff()
end, false)
