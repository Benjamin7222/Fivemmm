...
NewKaizo.netRegisterAndHandle('esx_vangelico_robbery:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

NewKaizo.netRegisterAndHandle('esx_vangelico_robbery:killblip', function()
    RemoveBlip(blipRobbery)
end)

NewKaizo.netRegisterAndHandle('esx_vangelico_robbery:setblip', function(position)
...
NewKaizo.netRegisterAndHandle('esx_vangelico_robbery:toofarlocal', function(robb)
	holdingup = false
...
NewKaizo.netRegisterAndHandle('esx_vangelico_robbery:robberycomplete', function(robb)
	holdingup = false
	...
	ESX.ShowNotification('~g~ Braquage terminé.~s~  Fuis !')
	store = ""
	incircle = false
end)
	...
	while true do
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						...
						if IsPedShooting(PlayerPedId()) then
							...
							KaizoClientUtils.toServer('esx_vangelico_robbery:rob', k, token)
                        end
		...
		if holdingup then
							...
							if IsControlJustReleased(1, 51) then
								...
								KaizoClientUtils.toServer('ewen:rageui')
								KaizoClientUtils.toServer('esx_vangelico_robbery:gioielli1', token)
                            ...
                            hasrobbed19 = false
                            hasrobbed20 = false
							KaizoClientUtils.toServer('esx_vangelico_robbery:endrob', store)
							ESX.ShowNotification('~g~Vous avez volé des bijoux ! Maintenant, apportez des bijoux à ~g~Lester!')
							TriggerEvent('skinchanger:getSkin', function(skin)
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -622.566, -230.183, 38.057, true) > 11.5 ) then
				KaizoClientUtils.toServer('esx_vangelico_robbery:toofar', store)
				holdingup = false
				RemoveBlip(gioielli1)
				RemoveBlip(gioielli2)
				RemoveBlip(gioielli3)
...
NewKaizo.netRegisterAndHandle('esx_vangelico_robbery:togliblip', function(robb)
	RemoveBlip(gioielli1)
	RemoveBlip(gioielli2)
	RemoveBlip(gioielli3)
	RemoveBlip(gioielli4)
...
function MenuVente()
	local vente = RageUI.CreateMenu("Black Marquet", "Vente Bijoux")
	...
	while vente do
		...
		RageUI.IsVisible(vente, function()
			RageUI.Button('Vendre Bijoux', nil, {}, true, {onSelected = function() KaizoClientUtils.toServer('lester:vendita') end});
		end)
