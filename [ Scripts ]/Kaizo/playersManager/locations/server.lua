local listeLocation = {
    ...
    ['bike'] = {name = 'bike', label = 'Bicyclette du bled', price = 100},
    ['club'] = {name = 'club', label = 'Club', price = 500},
    ['weevil'] = {name = 'weevil', label = 'Coccinelle', price = 800},
    ['michelli'] = {name = 'michelli', label = 'Michelli', price = 1000},
}
    ...
    if listeLocation[name] == nil then
        ...
        if listeLocation[name].name == name and listeLocation[name].price == tonumber(price) then
            if bank >= listeLocation[name].price then
                ...
                TriggerClientEvent('esx:showNotification', source, "" .. price .. "$ vous ont été prélevez de votre compte en banque. Bonne route !")
            end
        else
            DropPlayer(source, 'Problème de synchronisation avec les locations de véhicules')
        end
