...
ESX.RegisterServerCallback("okokBanking:GetPlayerInfo", function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', {
		...
		local data = {
			...
			playerIBAN = db.iban,
			walletMoney = xPlayer.getAccount('cash').money,
			sex = db.sex,
		}
...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getAccount('cash').money)
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous avez déposé "..amount.."$", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous n'avez pas beaucoup d'argent sur vous", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getAccount('cash').money)
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous avez retiré "..amount.."$", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous n'avez pas beaucoup d'argent sur la banque", 5000, 'error')
	end
end)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
				    	...
				    	TriggerClientEvent('okokBanking:updateTransactions', xPlayers[i], xTarget.getAccount('bank').money, xPlayer.getAccount('cash').money)
				    	TriggerClientEvent('okokNotify:Alert', xPlayers[i], "Banque", "Vous avez reçu "..amount.."$ de "..xPlayer.getName(), 5000, 'success')
				    end
				end
				TriggerEvent('okokBanking:AddTransferTransaction', amount, xTarget, source)
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getAccount('cash').money)
				TriggerClientEvent('okokNotify:Alert', source, "Banque", "Tu as transféré "..amount.."$ à "..xTarget.getName(), 5000, 'success')
			elseif xTarget == nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getAccount('cash').money)
				TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous avez transféré "..amount.."$ à "..targetName, 5000, 'success')
		...
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous ne pouvez pas vous envoyer d'argent", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getAccount('cash').money)
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous avez déposé "..amount.."$ à "..societyName, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous n'avez pas beaucoup d'argent sur vous", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('okokNotify:Alert', _source, "Banque", "Quelqu'un se retire déjà", 5000, 'error')
		else
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', _source, xPlayer.getAccount('cash').money)
			TriggerClientEvent('okokNotify:Alert', _source, "Banque", "Vous avez retiré "..amount.."$ de "..societyName, 5000, 'success')
		end
	else
		TriggerClientEvent('okokNotify:Alert', _source, "Banque", "Votre société n'a pas beaucoup d'argent sur la banque", 5000, 'error')
	end
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.getAccount('bank').money
		...
		if amount <= playerMoney then
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getAccount('cash').money)
			TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous avez transféré "..amount.."$ à "..societyName, 5000, 'success')
		else
			TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous n'avez pas beaucoup d'argent sur la banque", 5000, 'error')
		end
end)
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getAccount('cash').money)
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous avez transféré "..amount.."$ à "..societyNameTarget, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "Votre société n'a pas beaucoup d'argent sur la banque", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:TransferMoneyToPlayerFromSociety", function(amount, ibanNumber, targetIdentifier, acc, targetName, society, societyName, societyMoney, toMyself)
	...
	if amount <= societyMoney then
		...
		if xTarget ~= nil then
			...
			if not toMyself then
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
			    		...
			    		TriggerClientEvent('okokBanking:updateTransactions', xPlayers[i], xTarget.getAccount('bank').money, xTarget.getAccount('cash').money)
			    		TriggerClientEvent('okokNotify:Alert', xPlayers[i], "Banque", "Vous avez reçu "..amount.."$ de "..xPlayer.getName(), 5000, 'success')
				    end
				end
			end
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getAccount('cash').money)
			TriggerClientEvent('okokNotify:Alert', source, "Banque", "Vous avez transféré "..amount.."$ à "..xTarget.getName(), 5000, 'success')
		elseif xTarget == nil then
			local playerAccount = json.decode(acc)
			playerAccount.bank = playerAccount.bank + amount
			playerAccount = json.encode(playerAccount)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if Config.IBANChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerEvent('okokBanking:AddTransferTransactionToSociety', amount, source, "Banque", "Bank (IBAN)")
		TriggerClientEvent('okokBanking:updateIban', source, iban)
		TriggerClientEvent('okokBanking:updateIbanPinChange', source)
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "IBAN successfully changed to "..iban, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "You need to have "..Config.IBANChangeCost.."€ in order to change your IBAN", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if Config.PINChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerEvent('okokBanking:AddTransferTransactionToSociety', amount, source, "Banque", "Bank (PIN)")
		TriggerClientEvent('okokBanking:updateIbanPinChange', source)
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "PIN successfully changed to "..pin, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "Banque", "You need to have "..Config.PINChangeCost.."€ in order to change your PIN", 5000, 'error')
	end
end)