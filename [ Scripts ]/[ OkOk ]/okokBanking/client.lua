    ...
    while true do
        ...
        if NearBank() and not isBankOpened and NearBank() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("E", "pour ouvrir la banque")
            else
            	inRange = true
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] pour acceder a votre banque', 'darkblue', 'left') 
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("E", "pour accéder à l'ATM")
            else
            	inRange = true
            end
            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						exports['okokNotify']:Alert("BANK", "Tu dois activer ton code PIN a la banque Principal", 5000, 'info')
					end
				end)
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] Pour accdeder a Votre ATM', 'darkblue', 'left') 
        elseif not inRange and shown then
        	shown = false
        	exports['okokTextUI']:Close()
        end
