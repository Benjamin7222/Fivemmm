...
AddEventHandler(((_Prefix.."%s"):format(Events.saveOutfit)), function(type)
    ...
    if not (_ClotheConfig.california) then playerAccount = xPlayer.getAccount('cash').money else playerAccount = xPlayer.getAccount('cash').money end
    ...
    if (playerAccount) < (price) then
        TriggerClientEvent(_ClotheConfig.showNotifEvent, _src, "Il semblerait que vous ne possédiez pas l'argent nécessaire.")
        TriggerClientEvent(((_Prefix.."%s"):format(Events.closeAll)), _src)
        return
    end
    ...
    TriggerClientEvent(_ClotheConfig.showNotifEvent, _src, ("%s $ vous ont été prélevés de votre portefeuille pour l'achat de votre %s."):format(price, sampling))
    TriggerClientEvent(_ClotheConfig.showAdvancedNotifEvent, _src, "Hailey", "Vendeuse", selectedMessage, "CHAR_ANTONIA", 8)
...
AddEventHandler(((_Prefix.."%s"):format(Events.buyOutfit)), function(price, name)
    ...
    local randomDraw = math.random(1 ,3)
    local selectedMessage = messages[randomDraw]
    TriggerClientEvent(_ClotheConfig.showNotifEvent, _src, ("%s $ vous ont été prélevés de votre portefeuille pour l'achat de %s."):format(price, name))
    TriggerClientEvent(_ClotheConfig.showAdvancedNotifEvent, _src, "Hailey", "Vendeuse", selectedMessage, "CHAR_ANTONIA", 8)
    TriggerClientEvent(((_Prefix.."%s"):format(Events.saveSkin)), _src)
