								...
								onSelected = function(indexface, Items)
									ESX.TriggerServerCallback('Ewen:GetMoneyOutfit', function(hasEnoughMoney)
										if hasEnoughMoney then
											...
											ESX.ShowNotification('~w~Vous n\'avez pas assez d\'argents')
										end
									end)
								end,
								onActive = function()
        ...
        RageUI.IsVisible(menu, function()
            RageUI.Button('Sauvegarder la tenue', nil, {}, true, {
                onSelected = function()
					...
					ESX.ShowNotification('~w~Vous avez sauvegarder la tenue : ~p~'..LabelTenue.. '~w~ pour ~p~500$')
					RageUI.CloseAll()
                end,
            });
...
function openValidate()
    ...
    while face do
        ...
        RageUI.IsVisible(face, function()
			...
			RageUI.Button('~y~Validé le personnage', nil, {}, true, {
				onSelected = function() 
					TriggerEvent('skinchanger:getSkin', function(skin)
						TriggerServerEvent('esx_skin:save', skin)
						TriggerEvent('skinchanger:loadSkin', skin)
					...
					RageUI.CloseAll()
					ESX.ShowNotification('~w~Bienvenue sur le serveur bon jeu à vous !')
				end
			});
        end, function()
    end)
